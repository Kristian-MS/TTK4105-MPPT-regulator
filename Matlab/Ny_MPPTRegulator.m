...
t_e_ff = out.e_FF_irr.Time;
e_ff   = out.e_FF_irr.Data;
...
hold on
plot(t_e_ff, e_ff, '--', 'LineWidth', 2)
grid on
xlabel('Time [s]', 'FontSize', 13)
ylabel('Error', 'FontSize', 13)
title('Error comparison: FB vs FF+FB', 'FontSize', 14)
legend('Feedback error', 'FF+FB error', 'FF error', 'Location', 'best')
set(gca, 'FontSize', 12)
...
e_ff_interp = interp1(t_e_ff, e_ff, t_common, 'linear', 'extrap');

IAE_fb = cumtrapz(t_common, abs(e_fb_interp));
IAE_f  = cumtrapz(t_common, abs(e_f_interp));
IAE_ff = cumtrapz(t_common, abs(e_ff_interp));
